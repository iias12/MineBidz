...
namespace MineBidz.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(RegisterModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    //Save Packege info if Any
                    int userId = WebSecurity.GetUserId(model.UserName);
                    repository.SaveUserInfo(model.CompanyInfo, userId, model.PaymentTypeId, model.LogoPackageId, model.BiddingPackageId);
                    WebSecurity.Login(model.UserName, model.Password);

                    string subject = "MineBidz Registration";
                    string body = String.Format("User {0} requested with e-mail {1}, please review the request. Company name: {2}, Contact name: {3}, Phone: {4}, Mobile: {5}.",
                        model.UserName, model.CompanyInfo.Email, model.CompanyInfo.CompanyName, model.CompanyInfo.ContactName, model.CompanyInfo.Phone, model.CompanyInfo.Mobile);
                    if (model.LogoPackageId.HasValue)
                    {
                        Package pac = repository.GetPackage(model.LogoPackageId.Value);
                        body += String.Format(" Logo package Requested: {0}.", pac.PackageText);
                    }

                    if (model.BiddingPackageId.HasValue)
                    {
                        Package pac = repository.GetPackage(model.BiddingPackageId.Value);
                        body += String.Format(" Bidding package Requested: {0}.", pac.PackageText);
                    }
                    ...
                    if (model.PaymentTypeId.HasValue)
                    {
                        ...
                        body += String.Format(" Payment Type: {0}.", paymentType);
                    }
                    ...
                    try
                    {
                        ...
                        Utilities.SendMail("mike@iias.com", subject, body);
                    }
                    catch (Exception exc)
                    {
                    }
        ...
        public ActionResult Confirm()
        {
            ...
            List<Package> list = repository.ListPackageForUser(userId);
            if (list.Count > 0)
            {
